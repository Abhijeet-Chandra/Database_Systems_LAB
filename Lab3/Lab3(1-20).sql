...
SQL> SELECT c.course_id, COUNT(sec.course_id)
  2  FROM course c
  3  LEFT JOIN section sec
  4  ON c.course_id = sec.course_id AND sec.year = 2009
  5  GROUP BY c.course_id
  6  HAVING COUNT(sec.course_id) <= 1;
